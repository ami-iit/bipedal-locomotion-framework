...
public:
     ...
     */
    // clang-format on
    static std::unique_ptr<UkfMeasurement>
    ...
    /**
     * @brief getMeasurementVariableHandler access the `System::VariablesHandler` instance created
     * during the initialization phase.
     * @return the measurement variable handler containing all the measurement variables and their
     * sizes and offsets.
     */
    System::VariablesHandler& getMeasurementVariableHandler();

    /**
     * @brief predictedMeasure predict the new measurement depending on the state computed by the
     * predict step.
    ...
    predictedMeasure(const Eigen::Ref<const Eigen::MatrixXd>& cur_states) const override;
    ...
    /**
     * @brief innovation computes the innovation step of the ukf update as the difference between
     * the predicted_measurement and the measurement.
     * @param predicted_measurements is a `blf::Data` reference representing the measurement
     * predicted in the update step.
     * @param measurements is a `blf::Data` reference representing the measurements coming from the
     * user.
     * @return a `std::pair<bool, bfl::Data>` where the boolean value is always true and the
     * `bfl::Data` is the innovation term.
     */
    std::pair<bool, bfl::Data> innovation(const bfl::Data& predicted_measurements,
