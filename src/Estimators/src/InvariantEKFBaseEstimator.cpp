...
public:
     ...
     */
    bool calcExpHatX(const Eigen::VectorXd& vec, Eigen::MatrixXd& X);
    ...
    Eigen::Matrix<double, 6, 1> m_theta, m_deltaTheta; /**< placeholder for parameter update
                                                          variables */
...
bool InvariantEKFBaseEstimator::customInitialization(
    std::weak_ptr<BipedalLocomotion::ParametersHandler::IParametersHandler> handler)
{
    auto handle = handler.lock();
    if (handle == nullptr)
    {
        std::cerr << "[InvariantEKFBaseEstimator::customInitialization] The parameter handler has "
                     "expired. Please check its scope."
                  << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::customInitialization] Could not load options "
                     "related parameters."
                  << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::customInitialization] Could not load sensor "
                     "stddev related parameters."
                  << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::customInitialization] Could not load initial "
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::customInitialization] Could not load prior "
                     "stddev related parameters."
                  << std::endl;
        return false;
    }
    ...
    m_pimpl->constuctStateVar(m_priors, m_options.imuBiasEstimationEnabled, m_pimpl->m_P); // construct
                                                                                           // priors
...
bool InvariantEKFBaseEstimator::resetEstimator(
    const FloatingBaseEstimators::InternalState& newState,
...
void BipedalLocomotion::Estimators::InvariantEKFBaseEstimator::Impl::updateFilterMatrixDimensions(
    const bool& estimateBias)
{
    if (estimateBias)
    {
...
bool InvariantEKFBaseEstimator::predictState(const FloatingBaseEstimators::Measurements& meas,
                                             const double& dt)
{
    m_pimpl->calcSkewSymAtCurrenState(m_statePrev, m_pimpl->m_skew); // compute skews at priori
                                                                     // state
    ...
    if (!m_pimpl->propagateStates(meas,
                                  dt,
    ...
    m_pimpl->calcFc(m_statePrev,
                    m_pimpl->m_skew,
                    m_options.imuBiasEstimationEnabled,
                    m_pimpl->m_Fc); // compute Fc at priori state
    m_pimpl->calcQc(m_statePrev,
                    m_sensorsDev,
                    meas,
                    m_options.imuBiasEstimationEnabled,
                    m_pimpl->m_Qc); // compute Qc at priori state and previous measure
    m_pimpl->calcLc(m_statePrev,
                    m_pimpl->m_skew,
                    m_options.imuBiasEstimationEnabled,
                    m_pimpl->m_Lc); // compute Lc at priori state

    // discretize linearized dynamics and propagate covariance
    m_pimpl->m_Fk = m_pimpl->m_In + (m_pimpl->m_Fc * dt); // read as Fk = I + (Fc*dt)
    m_pimpl->m_Qk = (m_pimpl->m_Fk * m_pimpl->m_Lc * m_pimpl->m_Qc * (m_pimpl->m_Lc.transpose())
                     * (m_pimpl->m_Fk.transpose()))
                    * dt; // read as Qk = Fk Lc Qc Lc.T Fk.T
    m_pimpl->m_P = m_pimpl->m_Fk * m_pimpl->m_P * (m_pimpl->m_Fk.transpose())
                   + m_pimpl->m_Qk; // read as P = Fk P Fk.T + Qk
    m_pimpl->extractStateVar(m_pimpl->m_P,
                             m_options.imuBiasEstimationEnabled,
                                                 ...
                                                 const double& dt)
{
    ...
    m_modelComp.getIMU_H_feet(meas.encoders,
                              m_pimpl->IMU_H_LF,
                              m_pimpl->IMU_H_RF,
                              m_pimpl->J_IMULF,
                              m_pimpl->J_IMURF);
    ...
    //
    // An auxiliary matrix is used along with the Kalman gain due to a reduced form of state update
    // equations Pi = [I  0_{3x3}]
    if (meas.lfInContact && meas.rfInContact)
    {
        ...
        m_pimpl->m_obs << sIMU_p_RF, 0, 1, -1, 0, sIMU_p_LF, 0, 1, 0, -1;
        ...
        m_pimpl->m_measModelJacobian.block<3, 3>(firstFootPositionVecOffsetObs,
                                                 m_pimpl->m_vecOffsets.imuPosition)
            = -Eigen::Matrix3d::Identity();
        m_pimpl->m_measModelJacobian
            .block<3, 3>(firstFootPositionVecOffsetObs, m_pimpl->m_vecOffsets.rContactFramePosition)
            .setIdentity();
        m_pimpl->m_measModelJacobian
            .block<3, 3>(firstFootPositionVecOffsetObs, m_pimpl->m_vecOffsets.lContactFramePosition)
            .setZero();
        m_pimpl->m_measModelJacobian.block<3, 3>(secondFootPositionVecOffsetObs,
                                                 m_pimpl->m_vecOffsets.imuPosition)
            = -Eigen::Matrix3d::Identity();
        m_pimpl->m_measModelJacobian
            .block<3, 3>(secondFootPositionVecOffsetObs,
                         m_pimpl->m_vecOffsets.rContactFramePosition)
            .setZero();
        m_pimpl->m_measModelJacobian
            .block<3, 3>(secondFootPositionVecOffsetObs,
                         m_pimpl->m_vecOffsets.lContactFramePosition)
            .setIdentity();

        if (m_options.imuBiasEstimationEnabled)
        {
            m_pimpl->m_measModelJacobian
                .block<6, 6>(firstFootPositionVecOffsetObs, m_pimpl->m_vecOffsets.gyroBias)
                .setZero();
        }
        ...
        m_pimpl->m_auxMat.block<3, 3>(firstFootPositionVecOffsetObs, firstFootIGainOffset)
            = m_pimpl->m_auxMat.block<3, 3>(secondFootPositionVecOffsetObs, secondFootIGainOffset)
            = Eigen::Matrix3d::Identity();
        m_pimpl->m_auxMat.block<3, 11>(firstFootPositionVecOffsetObs, firstFootZeroGainOffset)
            .setZero();
        m_pimpl->m_auxMat.block<3, 7>(secondFootPositionVecOffsetObs, firstFootIGainOffset)
            .setZero();
        m_pimpl->m_auxMat.block<3, 4>(secondFootPositionVecOffsetObs, secondFootZeroGainOffset)
            .setZero();

        // prepare measurement noise covariance R
        m_pimpl->m_measNoiseVar.resize(linearizedDimension, linearizedDimension);
        m_pimpl->m_measNoiseVar.topLeftCorner<3, 3>()
            = A_R_IMU * m_pimpl->J_IMURF.topRows<3>() * Renc
              * (m_pimpl->J_IMURF.topRows<3>().transpose()) * (A_R_IMU.transpose());
        m_pimpl->m_measNoiseVar.topRightCorner<3, 3>().setZero();
        m_pimpl->m_measNoiseVar.bottomRightCorner<3, 3>()
            = A_R_IMU * m_pimpl->J_IMULF.topRows<3>() * Renc
              * (m_pimpl->J_IMULF.topRows<3>().transpose()) * (A_R_IMU.transpose());
    ...
    } else if (meas.rfInContact)
    {
        ...
        {
            m_pimpl->m_measModelJacobian
                .block<3, 6>(firstFootPositionVecOffsetObs, m_pimpl->m_vecOffsets.gyroBias)
                .setZero();
        }
        ...
        m_pimpl->m_auxMat.block<3, 3>(firstFootPositionVecOffsetObs, firstFootIGainOffset)
            = Eigen::Matrix3d::Identity();
        m_pimpl->m_auxMat.block<3, 4>(firstFootPositionVecOffsetObs, firstFootZeroGainOffset)
            .setZero();
    ...
    if (meas.lfInContact || meas.rfInContact)
    {
        if (!m_pimpl->updateStates(m_pimpl->m_obs,
                                   m_pimpl->m_measModelJacobian,
                                   m_pimpl->m_measNoiseVar,
                                   m_pimpl->m_auxMat,
                                   m_state,
                                   m_pimpl->m_P))
        {
...
bool InvariantEKFBaseEstimator::Impl::propagateStates(
    const FloatingBaseEstimators::Measurements& meas,
    const double& dt,
    const Eigen::Vector3d& g,
    const manif::SE3d& IMU_H_RF,
                                                   ...
                                                   Eigen::MatrixXd& P)
{
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::updateStates] Measurement model Jacobian size "
                     "mismatch"
                  << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::updateStates] Measurement noise covariance "
    ...
    m_innovation = m_BigX * obs; // minus bias terms which are zero in reduced form (Section III C
                                 // of the paper)
...
void InvariantEKFBaseEstimator::Impl::extractStateVar(
    const Eigen::MatrixXd& P,
    const bool& estimateBias,
    FloatingBaseEstimators::StateStdDev& stateStdDev)
{
    stateStdDev.imuOrientation
        = P.block<3, 3>(m_vecOffsets.imuOrientation, m_vecOffsets.imuOrientation)
              .diagonal()
              .array()
              .sqrt();
    stateStdDev.imuLinearVelocity
        = P.block<3, 3>(m_vecOffsets.imuLinearVel, m_vecOffsets.imuLinearVel)
              .diagonal()
              .array()
              .sqrt();
    stateStdDev.imuPosition = P.block<3, 3>(m_vecOffsets.imuPosition, m_vecOffsets.imuPosition)
                                  .diagonal()
                                  .array()
                                  .sqrt();
    stateStdDev.rContactFramePosition
        = P.block<3, 3>(m_vecOffsets.rContactFramePosition, m_vecOffsets.rContactFramePosition)
              .diagonal()
              .array()
              .sqrt();
    stateStdDev.lContactFramePosition
        = P.block<3, 3>(m_vecOffsets.lContactFramePosition, m_vecOffsets.lContactFramePosition)
              .diagonal()
              .array()
              .sqrt();
    if (estimateBias)
    {
        stateStdDev.gyroscopeBias
            = P.block<3, 3>(m_vecOffsets.gyroBias, m_vecOffsets.gyroBias).diagonal().array().sqrt();
        stateStdDev.accelerometerBias
            = P.block<3, 3>(m_vecOffsets.accBias, m_vecOffsets.accBias).diagonal().array().sqrt();
    }
}

void InvariantEKFBaseEstimator::Impl::constuctStateVar(
...
void InvariantEKFBaseEstimator::Impl::extractState(
                                                     ...
                                                     FloatingBaseEstimators::InternalState& state)
{
    ...
    {
        std::cerr << "[InvariantEKFBaseEstimator::constructState] State matrix does not seem to "
                     "have expected size of 7x7."
                  << std::endl;
        return false;
    }
...
bool InvariantEKFBaseEstimator::Impl::calcExpHatX(const Eigen::VectorXd& vec, Eigen::MatrixXd& X)
{
    ...
    if (vec.size() != 15)
    {
        std::cerr << "[InvariantEKFBaseEstimator::calcExpHatX] State vector does not seem to have "
                     "expected size of 15x1."
                  << std::endl;
        return false;
    }
                                                   ...
                                                   Eigen::MatrixXd& AdjX)
{
    ...
    AdjX.block<3, 3>(m_vecOffsets.imuOrientation, m_vecOffsets.imuOrientation)
        ...
        = AdjX.block<3, 3>(m_vecOffsets.lContactFramePosition, m_vecOffsets.lContactFramePosition)
        = R;
    ...
    AdjX.block<3, 3>(m_vecOffsets.rContactFramePosition, m_vecOffsets.imuOrientation)
        = (skew.prCross) * R;
    AdjX.block<3, 3>(m_vecOffsets.lContactFramePosition, m_vecOffsets.imuOrientation)
        = (skew.plCross) * R;
}
                                             ...
                                             Eigen::MatrixXd& Fc)
{
    ...
    if (estimateBias)
    {
        ...
        Fc.block<3, 3>(m_vecOffsets.rContactFramePosition, m_vecOffsets.gyroBias)
            = -(skew.prCross) * R;
        Fc.block<3, 3>(m_vecOffsets.lContactFramePosition, m_vecOffsets.gyroBias)
            = -(skew.plCross) * R;
    }
}
                                             ...
                                             Eigen::MatrixXd& Qc)
{
    ...
    Qc.block<3, 3>(m_vecOffsets.imuOrientation, m_vecOffsets.imuOrientation)
        = static_cast<Eigen::Matrix3d>(Qg.asDiagonal());
    Qc.block<3, 3>(m_vecOffsets.imuLinearVel, m_vecOffsets.imuLinearVel)
        = static_cast<Eigen::Matrix3d>(Qa.asDiagonal());

    if (estimateBias)
    {
        Eigen::Vector3d Qba = sensDev.accelerometerBiasNoise.array().square();
        Eigen::Vector3d Qbg = sensDev.gyroscopeBiasNoise.array().square();
        Qc.block<3, 3>(m_vecOffsets.gyroBias, m_vecOffsets.gyroBias)
            = static_cast<Eigen::Matrix3d>(Qbg.asDiagonal());
        Qc.block<3, 3>(m_vecOffsets.accBias, m_vecOffsets.accBias)
            = static_cast<Eigen::Matrix3d>(Qba.asDiagonal());
    }
    ...
    Qc.block<3, 3>(m_vecOffsets.rContactFramePosition, m_vecOffsets.rContactFramePosition)
        = R * static_cast<Eigen::Matrix3d>(Qrf.asDiagonal()) * (R.transpose());
    Qc.block<3, 3>(m_vecOffsets.lContactFramePosition, m_vecOffsets.lContactFramePosition)
        = R * static_cast<Eigen::Matrix3d>(Qlf.asDiagonal()) * (R.transpose());
}
                                             ...
                                             Eigen::MatrixXd& Lc)
{
    ...
    Lc.block(m_vecOffsets.imuOrientation,
             m_vecOffsets.imuOrientation,
             m_vecSizeWOBias,
             m_vecSizeWOBias)
        = AdjX;
    ...
    if (estimateBias)
    {
        Lc.block(m_vecOffsets.gyroBias, m_vecOffsets.gyroBias, 6, 6)
            = Eigen::Matrix<double, 6, 6>::Identity();
    }
}

void InvariantEKFBaseEstimator::Impl::calcSkewSymAtCurrenState(
    const FloatingBaseEstimators::InternalState& X, SkewSymContainter& skew)
{
                                                ...
                                                Eigen::MatrixXd& BigX)
{
    if (X.rows() != 7 && X.cols() != 7)
    {
        std::cerr << "[InvariantEKFBaseEstimator::copyDiagX] State matrix does not seem to have "
                     "expected size of 7x7."
                  << std::endl;
        return false;
    }
