                                    ...
                                    const System::VariablesHandler& variablesHandler)
{
    ...
    if (!ptr->getParameter("joint_torques_variable_name", jointTorquesVariableName)
        || !variablesHandler.getVariable(jointTorquesVariableName, m_jointsTorqueVariable))
    {
    ...
    if (m_jointsTorqueVariable.size != m_kinDyn->getNrOfDegreesOfFreedom())
    {
        std::cerr << errorPrefix
                  ...
                  << ", Joint Torque: " << m_jointsTorqueVariable.size << "." << std::endl;
        return false;
    }
...
bool JointsDynamicsTask::update()
{
